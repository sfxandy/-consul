...
# v0.4 — enforces selector-before-command order; graceful error with examples; refined usage; deduped dispatch.
# Goals:
...
COMMANDS:
  ...
  wait-ready       block until API is responsive (and print ACL gate)
...
cc(){ (
  cd "${DIR}" && eval "$(compose_cmd) -f \"${FILE}\" $*"
); }

sel_services(){
  s=""; [ "${SEL_SERVER}" = true ] && s="${s} consul-server"; [ "${SEL_AGENT}" = true ] && s="${s} consul-agent";
  if [ "${SEL_SERVER}" = false ] && [ "${SEL_AGENT}" = false ]; then s="consul-server consul-agent"; fi
  printf '%s
' "${s}" | awk '{$1=$1;print}'
}

curl_h(){ need curl; set -- -sS --max-time 5 "$@"; [ -n "${TOKEN}" ] && set -- "$@" -H "X-Consul-Token: ${TOKEN}"; curl "$@"; }
http_code(){ path="$1"; curl_h -o /dev/null -w '%{http_code}' "${HTTP%/}${path}"; }
http_get(){ path="$1"; curl_h "${HTTP%/}${path}"; }

check_alive(){ code="$(http_code "/v1/status/leader" || printf 000)"; [ "${code}" = 200 ]; }

check_acl_bootstrap_blocked(){ code="$(http_code "/v1/acl/bootstrap" || printf 000)"; case "${code}" in 403|405|409) return 0 ;; 200) return 1 ;; *) return 1 ;; esac }

wait_ready(){ end=$(expr "$(date +%s)" + "${TIMEOUT}"); while :; do now="$(date +%s)"; [ "${now}" -ge "${end}" ] && { err "timeout waiting for ${HTTP}"; return 1; }; if check_alive; then acl="open"; check_acl_bootstrap_blocked && acl="blocked"; info "api=up http=${HTTP} acl_bootstrap=${acl}"; return 0; fi; sleep 1; done }

# -------- actions --------
start(){ cc up -d $(sel_services); }
stop(){ cc stop $(sel_services); }
restart(){ stop || true; start; wait_ready; }
status(){ cc ps; if check_alive; then info "api=up http=${HTTP}"; else info "api=down http=${HTTP}"; fi }
ps_cmd(){ cc ps; }
logs(){ cc logs --no-log-prefix --tail=200 $(sel_services); }
tailf(){ cc logs -f --no-log-prefix $(sel_services); }
health(){ code="$(http_code "/v1/status/leader" || printf 000)"; leader="$(http_get "/v1/status/leader" 2>/dev/null || printf '')"; leader="$(printf '%s' "${leader}" | tr -d '
')"; acl="open"; check_acl_bootstrap_blocked && acl="blocked"; info "status_code=${code} leader=${leader} acl_bootstrap=${acl}"; }
exec_cmd(){ if [ "${SEL_SERVER}" = true ] && [ "${SEL_AGENT}" = false ]; then tgt=consul-server; elif [ "${SEL_AGENT}" = true ] && [ "${SEL_SERVER}" = false ]; then tgt=consul-agent; else err "exec requires exactly one of --server or --agent"; exit 2; fi; [ "$#" -gt 0 ] || { err "exec requires a command after --"; exit 2; }; cc exec "${tgt}" "$@"; }
check_secrets(){ refs=$(awk '/^secrets:/ {insec=1; next} insec && NF==0 {insec=0} insec {print}' "${FILE}" | awk -F: '{gsub(/ /,""); if ($1!="")} $1 ~ /[A-Za-z0-9_.-]+/ {print $1}' | sort -u); missing=0; present=0; missing_list=""; for s in ${refs}; do if podman secret exists "${s}" >/dev/null 2>&1; then present=$(expr ${present} + 1); else missing=$(expr ${missing} + 1); missing_list="${missing_list} ${s}"; fi; done; info "engine=podman compose_file=${FILE} present=${present} missing=${missing}"; [ "${missing}" -gt 0 ] && { err "Missing secrets:${missing_list}"; return 3; }; }

# -------- parse args (strict order: selectors before command) --------
ALL_ARGS="$*"
CMD=""; EXEC_ARGS=""; SEEN_CMD=false; WRONG_ORDER=false

# First pass: detect if user put a command *before* a selector
for tok in "$@"; do
  case "$tok" in
...
# Normal parse (selectors/flags first, then command)
while [ "$#" -gt 0 ]; do
  case "$1" in
    --dir) DIR="$2"; shift 2 ;;
    --file) FILE="$2"; shift 2 ;;
...
# Resolve location after parsing
[ -n "${DIR}" ] || DIR="$(find_dir)"
[ -n "${FILE}" ] || FILE="$(find_file)"

# For exec, arguments after -- are required; otherwise error
if [ "${CMD}" = "exec" ]; then
  if [ -z "${EXEC_ARGS}" ]; then
